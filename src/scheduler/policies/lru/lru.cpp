...
//contrutores
LRU::LRU(){}
LRU::LRU(list<Process> list_process, Cpu* cpu_ref, Memory* memory_ref, Storage* storage_ref, float quantum_time){
...
void LRU::check_finished(Process* current_process, int* quantum){
...
void LRU::check_process_in_pogress(Process* current_process){
    if(current_process == NULL) return;
...
void LRU::update_timestamp(Process** current_process){
...
void LRU::executing_process_cpu(Process* current_process){
    current_process->set_status_await();
    this->cpu_ref->set_process( current_process );
}

void LRU::executing_process_memory(Process* current_process){
...
void LRU::executing_process_storage(Process* current_process){
    current_process->set_status_block();
